...
Uses monkey-patching (not subclassing) because Python name-mangling makes
subclass __process_message overrides unreachable from parent thread methods.
"""
...
# ---------------------------------------------------------------------------
#  Diagnostic suffix un-packer (module-level helper)
# ---------------------------------------------------------------------------
...
def _diag_unpack_suffix(data: bytes, packet_size: int,
    ...
    Returns (bytes_consumed, FrameSuffixData).
    """
    ...
    print(f"  Suffix raw hex (first 64 bytes): {_hex(data)}")
...
def main():
    ...
    # ---- Monkey-patch __process_message ----
    # Python name-mangling: parent threads call self._NatNetClient__process_message(...)
    # A subclass override __process_message would be mangled to a *different* name
    # (_DiagnosticNatNetClient__process_message) and never called.
    #
    # Fix: replace the instance attribute _NatNetClient__process_message.
    # When stored as an instance attribute (not class attribute), the function
    # does NOT receive 'self' via the descriptor protocol - it is called directly
    # as fn(data, print_level).  This matches the thread call-site exactly.
    #
    # Save the original bound method before replacing it, so non-frame messages
    # can be delegated to the parent's handler.

    original_process_message = client._NatNetClient__process_message
    frame_counter = [0]  # mutable container so the wrapper can mutate it

    def diagnostic_wrapper(data: bytes, print_level=0):
        """Monkey-patched replacement for _NatNetClient__process_message.

        Called from both __command_thread_function and __data_thread_function.
        For NAT_FRAMEOFDATA messages, performs full diagnostic logging;
        otherwise delegates to the original handler.
        """
        major = client.get_major()
        minor = client.get_minor()
        message_id = get_message_id(data)
        packet_size = int.from_bytes(data[2:4], byteorder="little", signed=True)

        if message_id != client.NAT_FRAMEOFDATA:
            # Pass non-frame messages to the original handler unchanged
            return original_process_message(data, print_level)
        ...
        # 3) Peek at first 32 bytes of payload
        payload = data[4:]
        peek_len = min(32, len(payload))
        print(f"  Payload hex (first {peek_len} bytes): "
              f"{payload[:peek_len].hex(' ')}")

        # 4) Walk through mocap data sections using parent's name-mangled un-packers
        ...
        # -- Marker Set Data --
        rel, marker_set_data = client._NatNetClient__unpack_marker_set_data(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        mocap_data.set_marker_set_data(marker_set_data)

        # -- Legacy Other Markers --
        rel, legacy = client._NatNetClient__unpack_legacy_other_markers(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        ...
        rel, rigid_body_data = client._NatNetClient__unpack_rigid_body_data(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        ...
        # -- Labeled Marker Data --
        rel, labeled_data = client._NatNetClient__unpack_labeled_marker_data(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        mocap_data.set_labeled_marker_data(labeled_data)

        # -- Force Plate Data --
        rel, fp_data = client._NatNetClient__unpack_force_plate_data(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        mocap_data.set_force_plate_data(fp_data)

        # -- Device Data --
        rel, dev_data = client._NatNetClient__unpack_device_data(
            data[offset:], packet_size - offset, major, minor)
        offset += rel
        ...
        rel, suffix_data = _diag_unpack_suffix(
        ...
        client.current_frame_timestamp = suffix_data.timestamp
    ...
    # Install the monkey-patch on the instance
    client._NatNetClient__process_message = diagnostic_wrapper
