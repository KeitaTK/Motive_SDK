...
123.456,1,0.123,1.234,2.345,0.001,0.002,0.003,0.999
123.476,1,0.125,1.236,2.347,0.001,0.002,0.003,0.999
0,0,0,0,0,0,0,0,0
123.506,1,0.127,1.238,2.349,0.001,0.002,0.003,0.999
```

全0行は記録中の区切りマーカーとして手動挿入される（後述）。

### 9.4 記録操作（Enterキートグル）

`PythonSample.py` の `keyboard_monitor()`（`msvcrt` による非ブロッキングキー入力監視スレッド）で、Enter キー押下による3ステート状態遷移を実装する。

```
       Enter          Enter          Enter
None ────────▶ recording ────────▶ zero_inserted ────────▶ None
(待機)        (記録中)            (区切り行挿入済み)          (待機)
```

| 遷移 | アクション | 実装箇所 |
|------|-----------|---------|
| `None` → `recording` | `NatNetClient.start_recording()` を呼び出し、記録バッファを初期化 | PythonSample.py L137-143 |
| `recording` → `zero_inserted` | 全0行 `[0,0,0,0,0,0,0,0,0]` を `recording_data` に追加（記録区切りマーカー） | PythonSample.py L147-157 |
| `zero_inserted` → `None` | `NatNetClient.stop_recording()` を呼び出し、CSVファイルを保存 | PythonSample.py L159-168 |
