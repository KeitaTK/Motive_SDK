...
class NatNetClient:
    ...
    def __unpack_rigid_body( self, data, major, minor, rb_num):
        ...
        if new_id == 1:
            ...
            self.data_buffer[rb_num] = (
                new_id,
                [rel_x, rel_y, rel_z],  # ローカル座標系の位置
                [motive_qx, motive_qy, motive_qz, motive_qw],  # クオータニオン(x,y,z,w)
                self.data_No,
                data_time
            )
            
            print(f"Local pos: ({rel_x:.6f}, {rel_y:.6f}, {rel_z:.6f}), Quat: ({motive_qx:.6f}, {motive_qy:.6f}, {motive_qz:.6f}, {motive_qw:.6f})")
    ...
    def save_data(self):
        ...
        if data_list:
            ...
            new_id, local_pos, quat, data_no, data_time = rigid_body_data
    ...
    def stop_recording(self):
        """記録を停止"""
        self.recording = False
        print(f"Recording stopped. Total frames: {len(self.recorded_data)}")
